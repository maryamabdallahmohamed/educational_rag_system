...
class ContentProcessorAgent:
    """
    LangGraph / LangChain-compatible Content Processor Agent.

    Key changes (to fix "unsupported operand type(s) for +=: 'dict' and 'dict'"):
    - Wrap every handler tool so it returns a merge-safe dict: {"cpa_tool_output": "<json-string>"}.
      This prevents LangGraph from trying to merge nested dicts/lists into the internal state.
    - After agent execution, deserialize 'cpa_tool_output' to update CPA state in a controlled manner.
    """
    ...
    def __init__(self):
        ...
        self.prompt_template = PromptLoader.load_system_prompt("prompts/content_processor_agent.yaml")
    ...
    async def _check_rag_availability(self,query):
        ...
        try:
            ...
            logger.exception(f"Error checking RAG availability: {e}")
            self.current_state["documents_available"] = False
    ...
    async def process(self, query, document):
            ...
            query = query.strip()
            if not query:
                return {"answer": "I didn't receive any query. How can I help you?"}
            ...
            try:
                ...
                # 2. Try LangGraph/Runnable "messages" key
                elif "messages" in result and result["messages"]:
                    # Get the content of the last message (the AI response)
                    last_msg = result["messages"][-1]
                    answer = last_msg.content if hasattr(last_msg, "content") else str(last_msg)
                else:
                    answer = "Error: Agent returned an unexpected format."
